...
anna_directory = "https://annas-archive.org/search?index=&page=1&sort=&ext=pdf&display=&q=Python+Crash+Course%2C+3rd+Edition%3A+A+Hands-On%2C+Project-Based+Introduction+to+Programming"
...
for link in soup.find_all("a", class_="custom-a block mr-2 sm:mr-4 hover:opacity-80"):
    if len(book_link) == 3:
        print("Book link list is greater than 1. Exiting loop!")
        break
    else:
...
book_download_link = "https://annas-archive.org/slow_download/"

while True:
    if user_server_preference == "1":
        book_download_link += book_link[0][5:] + "/0/0"
        print(f"User requested download from this server {book_download_link}")
        break
    elif user_server_preference == "2":
        book_download_link += book_link[0][5:] + "/0/1"
        print(f"User requested download from this server {book_download_link}")
        break
    elif user_server_preference == "3":
        book_download_link += book_link[0][5:] + "/0/2"
        print(f"User requested download from this server {book_download_link}")
        break
    else:
